...
const send = function (type: any, payload: any) {
    ...
    if (process.env.TEST_TELEMETRY_WAIT === 'true') {
        return execa(...args, {
            stdio: 'inherit',
        });
    }
...
export const track = async function (eventName: string, payload = {}) {
    if (isCI) {
        return;
    }
    ...
    const { duration, status, ...properties }: any = payload;
    const defaultData = {
        event: eventName,
        userId,
        anonymousId: cliId,
...
export const identify = async function (payload: any) {
    if (isCI) {
        return;
    }
    const globalConfig = await getGlobalConfig();
