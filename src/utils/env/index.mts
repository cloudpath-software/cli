 ...
 */
export const findValueInValues = (values: any, context: any) => values.find((val: any) => {
 ...
 */
export const filterEnvBySource = (env: any, source: any) => Object.fromEntries(Object.entries(env).filter(([, variable]: any) => variable.sources[0] === source));
/**
 ...
 */
const fetchEnvelopeItems = async function ({ accountId, api, key, siteId }: any) {
 ...
 * }
 */
export const formatEnvelopeData = ({ context = 'dev', envelopeItems = [], scope = 'any', source }: any) => envelopeItems
    // filter by context
    .filter(({ values }: any) => Boolean(findValueInValues(values, context)))
    // filter by scope
    .filter(({ scopes }: any) => (scope === 'any' ? true : scopes.includes(scope)))
    // sort alphabetically, case insensitive
    .sort((left: any, right: any) => (left.key.toLowerCase() < right.key.toLowerCase() ? -1 : 1))
    // format the data
 ...
 */
export const getEnvelopeEnv = async ({ api, context = 'dev', env, key = '', raw = false, scope = 'any', siteInfo }: any) => {
    ...
    if (raw) {
        ...
        return entries.reduce((obj, [envVarKey, metadata]: any) => ({
            ...obj,
 ...
 */
export const getHumanReadableScopes = (scopes: any) => {
    const HUMAN_SCOPES = ['Builds', 'Functions', 'Runtime', 'Post processing'];
    const SCOPES_MAP: any = {
    ...
    if (!scopes) {
        // if `scopes` is not available, the env var comes from cloudpath.toml
        // env vars specified in cloudpath.toml are present in the `builds` and `post_processing` scope
    ...
    return scopes.map((scope: any) => SCOPES_MAP[scope]).join(', ');
};
/**
 ...
 */
export const translateFromEnvelopeToMongo = (envVars: any[] = [], context = 'dev') => envVars
    .sort((left: any, right) => (left.key.toLowerCase() < right.key.toLowerCase() ? -1 : 1))
    .reduce((acc, cur: any) => {
    const envVar = cur.values.find((val: any) => [context, 'all'].includes(val.context_parameter || val.context));
