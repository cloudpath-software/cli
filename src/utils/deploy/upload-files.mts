...
const uploadFiles = async (api: any, deployId: string, uploadList: any, { concurrentUpload, maxRetry, statusCb }: any) => {
    ...
    const uploadFile = async (fileObj: any, index: number) => {
        ...
        switch (assetType) {
            case 'file': {
                ...
                response = await retryUpload(() => api.hosting.deploys.id(deployId).uploadFile(file, encodeURI(normalizedPath)), maxRetry);
                console.log(file.name, (response as any).data)
                break;
            }
            default: {
