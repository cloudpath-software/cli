 ...
 * @param {import('../../commands/base-command.mts').default} program
 * @returns {void}
 */
const generateAutocompletion = (program: any) => {
    try {
        const autocomplete = program.commands.reduce((prev: any, cmd: any) => ({
            ...prev,
            [cmd.name()]: {
                name: cmd.name(),
                ...
                options: cmd.options
                    ...
                    .sort(sortOptions)
                    .map((opt: any) => ({ name: `--${opt.name()}`, description: opt.description })),
            },
        }), {});
