...
import { DEVERR, DEV_WARN, exit, log } from '../utils/command-helpers.mjs';
...
export const tryGetAgent = async ({ certificateFile, httpProxy }: any) => {
    if (!httpProxy) {
        return {};
    }
    let proxyUrl;
...
export const getAgent = async ({ certificateFile, httpProxy }: any) => {
    const { agent, error, message, warning }: any = await tryGetAgent({ httpProxy, certificateFile });
    if (error) {
        log(DEVERR, error, message || '');
        exit(1);
    }
    if (warning) {
        log(DEV_WARN, warning, message || '');
    }
    return agent;
};